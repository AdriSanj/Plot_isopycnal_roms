...
# opciones
# nc_water, gsw, unesco_water, opendrift_water, egg_dens, diff_gsw, diff_unesco, diff_gsw_unesco
TIPO_DENSIDAD = 'unesco_water'

print(
    f'Plotting (Modo: {TIPO_DENSIDAD}) con isopicnas de fondo (OpenDrift)...'
)
...
for fecha_target in fechas_evaluacion:
  for esp in especies:
    ...
    if len(indices_seleccionados) > 0:
      ...
      dens_opendrift = sea_water_density(T=t_sel, S=s_sel)

      if TIPO_DENSIDAD == 'gsw':
        c_p = dens_gsw
      elif TIPO_DENSIDAD == 'unesco_water':
        c_p = dens_unesco
      elif TIPO_DENSIDAD == 'opendrift_water':
        c_p = dens_opendrift
...
for num_progreso, fecha_target in enumerate(fechas_a_procesar, start=1):
  ...
  # --- DENSIDAD DE FONDO (ECUACIÓN OPENDRIFT) ---
  rho_sec = sea_water_density(T=temp_sec, S=salt_sec)

  # --- FILTRADO DE PARTÍCULAS DEL FRAME ---
