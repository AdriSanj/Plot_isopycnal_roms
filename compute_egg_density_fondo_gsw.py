...
# Configuración unificada de color y escalas fijas
shared_cmap = 'turbo'
vmin_iso = 1024.5
vmax_iso = 1027.5
num_isopicnas = 25
...
for num_progreso, fecha_target in enumerate(fechas_a_procesar, start=1):
  ...
  # Geometría vertical para el contorno
  ...
  # --- FILTRADO Y CÁLCULO DE PARTÍCULAS ---
  ...
  # Configuración de paleta y límites para la barra de color del scatter
  ...
  # Densidad de fondo (ROMS con GSW)
  ...
  #Determinar primero la ventana horizontal visible (X)
  ...
  # filtrar los datos de densidad dentro del encuadre visible (0-100m y X min-max)
  mask_visible = (
      (coord_2d >= x_min_plot)
      & (coord_2d <= x_max_plot)
      & (depth_sec >= depth_min)
  ...
  # repartir el número exacto de isopicnas (`num_isopicnas`) en la ventana
  ...
  # dibujar contornos manteniendo vmin_iso y vmax_iso para la coincidencia de color
  ax.contour(
      coord_2d,
      depth_sec,
      rho_masked,
